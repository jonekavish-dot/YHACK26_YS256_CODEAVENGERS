...
- **Why is it needed?**
  To rigorously evaluate MIRA against standard ROS2 Nav2 / Euclidean A* navigation without relying on hand-wavy marketing claims. Evaluators demand rigorous, reproducible physics simulations where both the baseline and MIRA face identical dynamic disturbances.
- **How does it work?**
...
| :---: | :--- | :--- | :--- | :--- |
| **1** | **MISSION** | *"This is the robot mission and its destination: R01 departs the Depot to deliver emergency medical supplies to the Field Station."* | Launch Mission (`EMERGENCY_DELIVERY`) | Robot sets off along nominal trajectory at $1.0\text{ m/s}$. Risk Gauge indicates **GREEN** (nominal range). |
...
| **3** | **FAULT** | *"Now we introduce real-world subsystem and environment degradation: a dynamic obstacle obstructs the corridor, and perception health drops."* | Click `[Dynamic Obstacle]` + `[Degrade Sensor]` | Obstacle appears ahead on corridor. Camera perception degrades below nominal threshold; spatial and sensor risk escalate. |
| **4** | **RISK** | *"MIRA detects the changing operating risk: the multi-factor engine continuously recalculates risk weights in real time."* | Observe Risk Breakdown Panel | Composite risk increases into high caution range. Perception and Proximity factors illuminate on the breakdown panel. |
| **5** | **DECISION** | *"The Safety Governor explains and chooses the safer operational response: throttling speed when sensing degrades, then rerouting when the corridor is obstructed."* | Click `[Degrade Sensor]`, then `[Dynamic Obstacle]` | Perception degradation triggers **ACTION**: `SLOW_DOWN`. When corridor is obstructed, Governor commands **ACTION**: `REPLAN`. **WHY**: *Sensor degraded below safe threshold + path obstructed*. **TRADEOFF**: *Selected bypass corridor is longer but lower-risk, avoiding obstacle impact*. |
| **6** | **PLANNER** | *"The planner adapts the trajectory when necessary, generating a safe bypass corridor away from the hazard zone."* | Live Grid Canvas replan | Blue trajectory automatically arcs around the obstacle with safe obstacle clearance. |
| **7** | **RESULT** | *"The robot either continues safely or aborts safely. Here, the robot bypasses the obstacle with zero contact and safely delivers supplies."* | Click `[Recover Subsystems]` | Subsystems recover, robot arrives safely at Medical Camp. **Collision avoided in tested run**. |
| **8** | **EVIDENCE** | *"Here is the empirical benchmark comparison: 20 paired simulation trials show MIRA achieves 0 collisions in tested trials (0 vs 3) and reduces physical risk exposure by 91.4% (from 25.5 to 2.2 pts)."* | Switch to **Baseline vs MIRA** Tab & Click `[Run Benchmark]` | Live Monte Carlo runs 20 paired trials: Baseline incurs 3 collisions and 37 near-misses (85% arrival); MIRA achieves 0 collisions and 3 near-misses (100% arrival), reducing risk exposure from 25.5 to 2.2 pts. |
...
2. **"What if the battery depletes halfway through?"**
   $\to$ Select *Battery Low $\to$ RETURN_TO_SAFE_ZONE*: Evaluates energy floor ($25\%$) and commands safe abort to safe zone depot rather than stranding.
3. **"What if wireless communication drops completely?"**
   $\to$ Select *Comms Drop $\to$ DEGRADED_AUTONOMY*: Switches to local onboard fail-safe autonomy policy (`DEGRADED_AUTONOMY`) with widened safety margins.
